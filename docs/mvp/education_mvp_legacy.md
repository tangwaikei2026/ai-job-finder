...
| `education_major_prefer`   | `list[string]` | `["computer", "design"]`     | 必须    | 判断专业偏好，不直接 reject            |
...
## education_degree_barrier 可比较的优先级：

```text
associate_plus < bachelor_plus < master_plus < phd_plus
```

### 补充

```text
none = 不参与学历等级比较，只表示岗位明确学历不限，无学历硬门槛
unknown = 不参与学历等级比较，不能直接 pass/reject
```
...
## education_degree_prefer 可比较的优先级：
...
只有专业词出现在**硬要求区域**，才进 `education_major_barrier`。
...
一个岗位聚合 API education、requirements 和 description 后，education 信息是否能够稳定写入 clean。
...
- API/text_rule 有冲突，最终为 conflict。
...
{
  "education_degree_barrier": "bachelor_plus",
  "education_major_prefer": ["computer", "marketing"]
}
```
...
```json
{
  "education_degree_barrier": "unknown",
  "education_degree_prefer": [],
  "education_major_barrier": ["computer", "algorithm", "math_stats"],
  "education_major_prefer": [],
  "education_parse_status": "ok",
  "education_source": {
    "degree_barrier": "none",
