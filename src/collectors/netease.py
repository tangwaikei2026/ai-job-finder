...
class NeteaseRawCollector(RawCollector):
    platform = "netease"
        ...
        try:
            while True:
                ...
                if payload.get("code") != 200:
                    raise RuntimeError(
                        f"NetEase API returned code={payload.get('code')}: "
                        f"{payload.get('msg', '')}"
                    )
                ...
                page_ids = tuple(str(item.get("id", "")) for item in records)
    ...
    def _map_job(self, item: dict[str, Any]) -> RawJobPosting:
        ...
        work_places = item.get("workPlaceNameList") or []
        if isinstance(work_places, list):
            location = ", ".join(str(place) for place in work_places if place)
        else:
            location = str(work_places)

        source_url = str(item.get("beeUrl") or "")
        ...
        return RawJobPosting(
            ...
            company=str(self.platform_config.get("name") or "网易"),
            department=str(item.get("firstDepName") or ""),
            location=location,
            experience=str(item.get("reqWorkYearsName") or ""),
            education=str(item.get("reqEducationName") or ""),
